...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const images = ev.poster ? [ev.poster] : [];
  return {
    title,
    description,
    openGraph: { title, description, images, type: 'event' },
...
export default function EventDetailPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    eventAttendanceMode:
      ev.mode === 'Webinar' ? 'https://schema.org/OnlineEventAttendanceMode' : 'https://schema.org/OfflineEventAttendanceMode',
    eventStatus: 'https://schema.org/EventScheduled',
    image: ev.poster ? [`${ev.poster}`] : [],
    // Si tienes fecha ISO, colócala aquí. Si no, se queda como string legible.
    startDate: ev.date,
    location:
  ...
  return (
    <section className="container py-10">
      ...
      <nav className="text-sm text-neutral-500 mb-4">
        <Link href="/eventos" className="hover:text-neutral-200">Eventos</Link>
      ...
      <header className="grid lg:grid-cols-[420px,1fr] gap-6 items-start">
        ...
        <div>
          ...
          <div className="mt-3 text-sm text-neutral-400 space-y-1">
            <div><span className="text-neutral-500">Fecha:</span> {ev.date}</div>
            {ev.time && <div><span className="text-neutral-500">Horario:</span> {ev.time}</div>}
            <div><span className="text-neutral-500">Modalidad:</span> {ev.mode}</div>
            <div><span className="text-neutral-500">Ubicación:</span> {ev.location}</div>
          </div>

          <p className="mt-4 text-neutral-200">{ev.summary}</p>

          <div className="mt-5 flex flex-wrap gap-3">
            <Link href={joinUrl} target="_blank" className="btn btn-primary">Inscribirme (socio)</Link>
            <Link href="/membresias" className="btn btn-secondary">Quiero ser socio</Link>
          </div>
          ...
          {ev.sponsors && ev.sponsors.length > 0 && (
            <div className="mt-6">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3">
                {ev.sponsors.map((s) => (
                  <div key={s.name} className="h-9 px-3 rounded-xl border border-neutral-800 bg-neutral-950 grid place-items-center">
                    <img src={s.logo} alt={s.name} className="max-h-7 object-contain opacity-80" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <ul className="divide-y divide-neutral-800">
              {ev.agenda.map((a, idx) => (
                <li key={idx} className="grid md:grid-cols-[110px,1fr,220px] gap-3 p-4">
                  ...
                  <div className="text-sm text-neutral-500">{a.speaker ?? ''}</div>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="mt-10 rounded-2xl border border-neutral-800 bg-neutral-900 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-400">Acceso y materiales exclusivos para socios APCC.</p>
        </div>
